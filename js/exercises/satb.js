...
export async function displaySATBExerciseOnStaff(exercise) {
  ...
  applyExerciseTempo(exercise); // adopt the hymn's own tempo (OpenPsalm tempo_bpm)
...
export function applyExerciseTempo(exercise) {
  ...
  const tempo = Math.max(40, Math.min(200, Math.round(bpm)));
...
function syncSatbTempoUI(tempo) {
  ...
  if (slider) {
    if (Number(slider.max) < tempo) slider.max = String(tempo);
