...
Tabs use `data-tab` HTML attributes. `js/ui/components/tabs.js` manages panel visibility. The global Play button in `js/ui/components/transport.js` routes to the correct exercise handler based on `appState.exercise.currentTab`.
...
- **Dark theme:** CSS custom properties in `styles/variables.css` (`--bg: #0f1220`, `--panel: #171a2b`, etc.).
